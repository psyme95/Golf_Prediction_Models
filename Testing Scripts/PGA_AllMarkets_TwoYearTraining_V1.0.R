...
MODEL_NAME <- paste0("MultiMarket_2YrTrain_", format(Sys.time(), "%d%m_%H%M"))
NUM_ITERATIONS <- 20  # Number of times to repeat the entire process
start_time <- Sys.time()
...
BETTING_MARKETS <- list(
  ...
  "Top20" = list(
    ...
    model_odds_cols = c("Top20_odds")  # Top20 + next level
  )
)
...
# Function to get year pairs for training/testing (MODIFIED FOR 2-YEAR TRAINING)
get_year_pairs <- function(df_all) {
  ...
  # Now we need at least 2 years for training, so start from index 3
  year_pairs <- list()
  for (i in 3:length(unique_years)) {
    year_pairs[[i-2]] <- list(
      train_years = c(unique_years[i-2], unique_years[i-1]),  # Two previous years
...
# Process single betting market with yearly validation (MODIFIED FOR 2-YEAR TRAINING)
process_market_yearly_validation <- function(market_name, market_config, df_all, 
  ...
  if (length(missing_vars) > 0) {
    ...
    if (length(available_vars) == 0) {
      stop("No variables available for modeling!")
    }
  }
  ...
  for (iteration in 1:NUM_ITERATIONS) {
    ...
    for (pair_idx in seq_along(year_pairs)) {
      train_years <- year_pairs[[pair_idx]]$train_years  # Now plural
      test_year <- year_pairs[[pair_idx]]$test_year
      
      cat("\n  Training on years", paste(train_years, collapse=" and "), 
          "-> Testing on year", test_year, "\n")
      
      # Get training data (two full years instead of one)
      train_data <- df_all %>%
        filter(year %in% train_years) %>%  # Use %in% for multiple years
      ...
      if (nrow(train_data) == 0) {
        cat("  WARNING: No training data available for years", 
            paste(train_years, collapse=" and "), ". Skipping.\n")
        next
      }
      
      if (nrow(test_data) == 0) {
        cat("  WARNING: No test data available for year", test_year, ". Skipping.\n")
        next
      }
      ...
      model_metrics <- model_result$metrics %>%
        mutate(
          iteration = iteration,
          train_years = paste(train_years, collapse="_"),  # Store both years
          test_year = test_year,
          market = market_name
        )
      ...
      calibration_model_glm_odds <- glm(
        Actual_Result ~ Model_Score + Market_Odds,
      ...
      calibration_model_glm_prob <- glm(
        Actual_Result ~ Model_Score + Implied_Prob,
      ...
      myBiomodProj <- BIOMOD_EnsembleForecasting(
        ...
        proj.name = paste0("iter", iteration, "_", 
                           paste(train_years, collapse="_"), "_to_", 
  ...
  if (length(all_iterations_results) > 0) {
    ...
    averaged_predictions <- all_predictions_combined %>%
      ...
      summarise(
        ...
        Model_Score_SD = sd(Model_Score, na.rm = TRUE),
        Model_Score_Min = min(Model_Score, na.rm = TRUE),
        Model_Score_Max = max(Model_Score, na.rm = TRUE),
        ...
        GLM_Odds_Probability_SD = sd(GLM_Odds_Probability, na.rm = TRUE),
        ...
        GLM_ImpliedProb_Probability_SD = sd(GLM_ImpliedProb_Probability, na.rm = TRUE),
...
data_path <- here("Weekly_Modelling", "Input", "Euro_Processed.xlsx")
...
for (pair in year_pairs) {
  cat("  Train:", paste(pair$train_years, collapse=" & "), 
      "-> Test:", pair$test_year, "\n")
}
...
for (market_name in names(market_results)) {
  ...
  cal_data_glm_odds <- create_calibration_data(
    ...
    "GLM_Odds_Probability_Mean", 
    "GLM (Odds)"
  )
  cal_data_glm_prob <- create_calibration_data(
    result$averaged_predictions, 
    "GLM_ImpliedProb_Probability_Mean", 
    "GLM (Implied Prob)"
  )
  cal_data_platt <- create_calibration_data(
    result$averaged_predictions, 
    "Platt_Probability_Mean", 
    "Platt Scaling"
  )
  ...
  calibration_plot <- ggplot(cal_data_combined, 
                             aes(x = predicted_prob, y = actual_rate, color = method)) +
    ...
    labs(
      ...
      title = paste(market_name, "Market - 2-Year Training (Mean Predictions)")
    ) +
    theme_minimal() +
...
if (length(market_results) > 1) {
  ...
  # Create comparison plot across markets
  comparison_plot <- ggplot(combined_metrics, 
                            aes(x = Market, y = Brier_Score, fill = Method)) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(
      title = "Calibration Performance Across Markets (2-Year Training)",
      x = "Market",
  ...
  ggsave(
    paste0(main_results_dir, "All_Markets_Brier_Score_Comparison.png"),
    comparison_plot,
    width = 10,
    height = 6,
    dpi = 300
...
for (market_name in names(market_results)) {
  ...
  summary_stats[[market_name]] <- data.frame(
    ...
    Training_Window = "2 years"  # Added to indicate training window
  )
}
...
cat("\nNOTE: Now using 2-year training windows instead of 1-year\n")