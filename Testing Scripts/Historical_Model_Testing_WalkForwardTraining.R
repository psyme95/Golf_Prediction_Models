...
MODEL_NAME <- paste0("YearlyWalkForward_", format(Sys.time(), "%d%m_%H%M"))
...
# Function to get year pairs for training/testing (MODIFIED FOR EXPANDING WINDOW)
get_year_pairs <- function(df_all) {
  ...
  # Create training/test year pairs with expanding window
  year_pairs <- list()
  for (i in 2:length(unique_years)) {
    year_pairs[[i-1]] <- list(
      train_years = unique_years[1:(i-1)],  # ALL previous years
...
process_market_yearly_validation <- function(market_name, market_config, df_all, 
  ...
  for (iteration in 1:NUM_ITERATIONS) {
    ...
    for (pair_idx in seq_along(year_pairs)) {
      ...
      train_data <- df_all %>%
        filter(year %in% train_years) %>%  # This already works with multiple years
...
for (market_name in names(market_results)) {
  ...
  summary_stats[[market_name]] <- data.frame(
    ...
    Training_Window = "Yearly Walk Forward"  # Added to indicate training window
  )
}
...
cat("Modelling completed at:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
end_time <- Sys.time()
