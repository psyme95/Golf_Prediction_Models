...
proj.list <- list()
selected_indices <- sample(1:length(pgalist_filtered), 10)
...
expl.proj <- PGA_projection[, !names(all_players_data) %in% c("eventID", "posn", "playerID","score", "win", "top_40", "Top20_Odds", "Top40_Odds")]
...
run  <-   "1.EQ71-72" # CHANGE THE RUN NUMBER
reps  <-  5 # CHANGE NO OF MODEL REPS. This should BE 5 fold for exploratory models, 1 fold for the final run.

models.to.proc = c('GAM','GLM','RF','FDA','CTA','ANN','GBM','MAXNET','XGBOOST') 

#For FULL models in  final runs (depending on the variables)
#FULL 10 models (no factors)
#models.to.proc = c('GAM','GLM','RF','FDA','CTA','ANN','GBM','MAXENT','SRE','MARS') 
#FULL model with factors
#models.to.proc = c('GAM','GLM','RF','FDA','CTA','ANN','GBM','MAXENT','MARS','MAXNET') 
#FULL Drop CTA  or other difficult ones 
# models.to.proc = c('GAM','GLM','RF','ANN','GBM','MAXENT.Phillips') 
#TRY TO INCLUDE THESE AS A MINIMUM
#models.to.proc = c('GAM','GLM','RF','GBM','MAXENT.Phillips') 
...
user.GAM <- list('_allData_allRun' = list( algo = 'GAM_mgcv',
                                           ...
                                           interaction.level = 0,    #Increase to 1 if trouble fitting models 
                                           myFormula = NULL,
...
source("A:/OneDrive - University of Southampton/Golf/cutoff_finder_biomod.R")
...
for (i in 1:length(PGA_Pred_list)) {
  ...
  PGA_Pred_list[[i]] <- merge(
    PGA_Pred_list[[i]],
    PGA_projection[, c("eventID", "playerID", "Top40_Odds", "posn")],
    by = c("eventID", "playerID"),
    all.x = TRUE
  )
  
  # Add actual Top40 finish column
  PGA_Pred_list[[i]]$Top40 <- ifelse(PGA_Pred_list[[i]]$posn <= 40, 1, 0)
  
  # Setup odds validity
  PGA_Pred_list[[i]]$Valid_Odds <- PGA_Pred_list[[i]]$Top40_Odds > 1
...
# Save the workbooks
summary_excel_file_path <- paste0("./Betting Simulations/PGA_Event_Betting_Outcomes_Model", run, ".xlsx")
bets_excel_file_path <- paste0("./Betting Simulations/PGA_Player_Betting_Outcomes_Model", run, ".xlsx")
