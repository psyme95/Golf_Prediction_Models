...
CV_REPETITIONS <- 1
CV_PERCENTAGE <- 0.7
MODEL_NAME <- paste0("Top20_RW_SmallEvents_", format(Sys.time(), "%d%m_%H%M"))
MAX_PLAYERS_PER_EVENT <- 119

# Rolling window parameters
ROLLING_WINDOW_SIZE <- 20  # Number of events to include in training window
...
# Save predictions
write.csv(all_predictions, paste0(results_dir, MODEL_NAME, "_rolling_window_predictions.csv"), row.names = FALSE)
...
if (nrow(all_predictions) > 0) {
  ...
  # Performance by training window size (showing how performance changes over time)
  window_performance <- all_predictions %>%
    group_by(Training_Events_Used) %>%
    summarise(
      events = n_distinct(EventNumber),
      players = n(),
      accuracy = mean(Prediction_Correct, na.rm = TRUE),
      avg_prob_edge = mean(Prob_Edge[Valid_Odds], na.rm = TRUE),
      .groups = 'drop'
    )
