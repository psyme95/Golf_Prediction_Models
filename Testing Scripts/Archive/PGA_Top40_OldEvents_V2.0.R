...
# df_old[is.na(df_old$Top20_Profit), "Top20_Profit"] <- 0
# df_old[is.na(df_old$Top40_Profit), "Top40_Profit"] <- 0
...
# train_event_list <- event_list[1:116]
...
for (event_idx in 1:length(new_events)) {
  ...
  event_data <- test_data %>%
    ...
    mutate(Top40_Odds = filter(df_new, eventID == test_event)$Top40_Odds,
           Top40_Profit = filter(df_new, eventID == test_event)$Top40_Profit,
  ...
  tryCatch({
    ...
    PGA_Prediction <- merge(
      PGA_Prediction,
      event_data[, c("eventID", "playerID", "Top40_Odds", "Top40_Profit", "posn", "top_40", "rating")],
    ...
    PGA_Prediction$Prediction_Correct <- ifelse(
      (PGA_Prediction$top_40 == 1 & PGA_Prediction$Model_Rank <= 40) | 
        (PGA_Prediction$top_40 == 0 & PGA_Prediction$Model_Rank > 40), 
      1, 0)    
    PGA_Prediction$Top40_Winnings <- ifelse(PGA_Prediction$top_40 == 1, PGA_Prediction$Top40_Profit, -10)
    ...
    prediction_record <- data.frame(
      ...
      Top40_Winnings = PGA_Prediction$Top40_Winnings + 10,
      Top40_Profit = PGA_Prediction$Top40_Winnings,
    ...
    for (strategy_name in names(strategies)) {
      ...
      if (sim$bankroll >= total_stake_needed) {
        # Place bets and calculate results using Top40_Profit column
        successful_bets <- sum(bet_data$top_40 == 1)
        # Use Top40_Profit directly - it already accounts for £10 stake and shows net profit
        total_winnings <- sum(ifelse(bet_data$top_40 == 1, bet_data$Top40_Profit, -FIXED_STAKE))
        ...
        gross_winnings <- sum(ifelse(bet_data$top_40 == 1, bet_data$Top40_Profit + FIXED_STAKE, 0))
