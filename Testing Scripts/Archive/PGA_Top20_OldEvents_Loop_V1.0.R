...
# Split events into list elements and randomly select 20% for testing
  ...
  # Save Excel file
  excel_file <- paste0("C:/Projects/Golf/Betting Simulations/Testing/Top20_Loop_", 
...
# Combine all excel outputs and calculate statistics
files <- list.files("./Betting Simulations/Testing", full.names = T, pattern = ".xlsx")
myfiles = lapply(files, read_excel, sheet="Model_Predictions")

summary_df <- bind_rows(myfiles) %>%
  group_by(Strategy) %>%
  summarise(
...
write.csv(summary_df, "./Betting Simulations/Testing/Simulation_Averages.csv", row.names = F)
