...
CV_REPETITIONS <- 1
CV_PERCENTAGE <- 0.7
MODEL_NAME <- paste0("Top5_RW_", format(Sys.time(), "%d%m_%H%M"))
...
get_rolling_window_data <- function(df_all, current_event, window_size, available_vars) {
  ...
  train_data <- df_all %>%
    ...
    select(playerID, eventID, posn, top_5, Top5_odds, all_of(available_vars))
...
simulate_kelly_betting_custom <- function(df, 
  ...
  for(event_idx in 1:length(unique_events)) {
    ...
    for(i in 1:nrow(event_data)) {
      ...
      if (is.na(row$Calibrated_Probability) || is.na(row$Top5_odds) || 
          row$Top5_odds <= 0 || !row$Valid_odds) {
        next
      }
      
      # Calculate probability edge over market
      implied_prob <- 1 / row$Top5_odds
      prob_edge <- row$Calibrated_Probability - implied_prob
      
      # Use pre-calculated Kelly fraction, but cap at 50%
      ...
      passes_filters <- (
        adjusted_kelly > min_kelly_threshold &&
          ...
          row$Top5_odds >= min_odds &&
          row$Top5_odds <= max_odds
      )
    ...
    if(nrow(potential_bets) > 0) {
      ...
      for(j in 1:nrow(potential_bets)) {
        ...
        won_bet <- original_row$Actual_Top5 == 1
        
        if(won_bet) {
          profit <- (bet_data$final_bet_amount / 10) * original_row$Top5_Profit
        } else {
...
for (current_event in MIN_PREDICTION_EVENT:total_events) {
  ...
  if (need_retrain || length(trained_models) == 0) {
    ...
    if (ENABLE_CALIBRATION) {
      cat("  Training calibration model on full training window...\n")
      ...
      tryCatch({
        ...
        logit_model_data <- data.frame(
          ...
          Actual_Top5 = train_data$top_5,
          Top5_odds = train_data$Top5_odds
        )
        
        # Train calibration model
        # temp_calibration_model <- glm(Actual_Top5 ~ Model_Score + Top5_odds,
        #                               data = logit_model_data,
        #                               family = binomial())
        temp_calibration_model <- glm(Actual_Top5 ~ Model_Score,
                                      data = logit_model_data,
                                      family = binomial())
  ...
  test_columns <- c("playerID", "eventID", "posn", "top_5", "Quality", 
                    "Top5_odds", "Top5_Profit", "rating", available_vars)
  ...
  tryCatch({
    ...
    essential_cols <- c("eventID", "playerID", "posn", "top_5")
    merge_cols <- intersect(names(test_data), c(essential_cols, "Top5_odds", "Top5_Profit", "rating", "Quality"))
    ...
    if (!is.null(calibration_model) && ENABLE_CALIBRATION) {
      ...
      calibration_data <- data.frame(
        ...
        Top5_odds = PGA_Prediction$Top5_odds
      )
    ...
    PGA_Prediction$Valid_odds <- PGA_Prediction$Top5_odds > 1
    PGA_Prediction$Model_Rank <- rank(-PGA_Prediction$Model_Score, ties.method = "first")
    PGA_Prediction$Prediction_Correct <- ifelse(
      (PGA_Prediction$top_5 == 1 & PGA_Prediction$Model_Rank <= 5) | 
        (PGA_Prediction$top_5 == 0 & PGA_Prediction$Model_Rank > 5), 
      1, 0)
    
    PGA_Prediction$Top5_Winnings <- ifelse(PGA_Prediction$top_5 == 1, PGA_Prediction$Top5_Profit, -10)
    
    # Add Kelly calculations
    PGA_Prediction <- PGA_Prediction %>%
      mutate(
        # odds are already in decimal format
        decimal_odds = Top5_odds,
    ...
    cat("  Successfully processed event", current_event, "- Players:", nrow(PGA_Prediction), 
        ...
        "Window:", prediction_record$Training_Window_Start[1], "to", prediction_record$Training_Window_End[1], "\n")
...
# Save predictions
write.csv(all_predictions, paste0(results_dir, MODEL_NAME, "_Model_Predictions.csv"), row.names = FALSE)
...
if (ENABLE_BETTING_SIMULATION && nrow(all_predictions) > 0) {
  ...
  write.csv(kelly_sim_rolling$bet_history, paste0(results_dir, MODEL_NAME, "_bet_history.csv"), row.names = FALSE)
  write.csv(kelly_sim_rolling$bankroll_history, paste0(results_dir, MODEL_NAME, "_bankroll_history.csv"), row.names = FALSE)
  ...
  write.csv(params_df, paste0(results_dir, MODEL_NAME, "_parameters.csv"), row.names = FALSE)
  
  # Create performance visualization
  if (require(ggplot2, quietly = TRUE)) {
    tryCatch({
      rolling_history <- kelly_sim_rolling$bankroll_history
      
      # Create comprehensive plot
      bankroll_plot <- ggplot(rolling_history, aes(x = event_number, y = bankroll_end)) +
      ...
      # Additional analysis: Performance stability over time
      if (nrow(kelly_sim_rolling$bankroll_history) > 10) {
        # Calculate rolling returns
        rolling_history$rolling_return <- c(0, diff(rolling_history$bankroll_end) / rolling_history$bankroll_start[-1])
        
        # Create returns plot
        returns_plot <- ggplot(rolling_history[-1, ], aes(x = event_number, y = rolling_return * 100)) +
          geom_line(alpha = 0.7) +
          geom_point(alpha = 0.5) +
          geom_hline(yintercept = 0, linetype = "dashed", color = "red", alpha = 0.7) +
          labs(
            title = "Event-by-Event Returns",
            subtitle = "Rolling Window Model Performance",
            x = "Event Number",
            y = "Return (%)"
          ) +
          theme_minimal()
        
        ggsave(paste0(results_dir, MODEL_NAME, "_rolling_returns.png"), 
               returns_plot, width = 12, height = 6, dpi = 300)
        
        cat("Rolling returns plot saved to:", paste0(results_dir, MODEL_NAME, "_rolling_returns.png"), "\n")
      }
      
    }, error = function(e) {
      cat("Could not create performance plots:", e$message, "\n")
    })
  }
}
