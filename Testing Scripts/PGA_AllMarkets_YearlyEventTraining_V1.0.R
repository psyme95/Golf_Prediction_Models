...
MODEL_NAME <- paste0("MultiMarket_Yearly_", format(Sys.time(), "%d%m_%H%M"))
...
# MODIFIED: Now properly includes market-specific odds as predictors
get_market_model_vars <- function(market_config, base_model_vars) {
  ...
  # This is the key change - now Win_odds will be included for the Winner market
...
process_market_yearly_validation <- function(market_name, market_config, df_all, 
  ...
  for (iteration in 1:NUM_ITERATIONS) {
    ...
    for (pair_idx in seq_along(year_pairs)) {
      train_year <- year_pairs[[pair_idx]]$train_year
      test_year <- year_pairs[[pair_idx]]$test_year
      
      cat("\n  Training on year", train_year, "-> Testing on year", test_year, "\n")
      ...
      if (nrow(train_data) == 0) {
        cat("  WARNING: No training data available for year", train_year, ". Skipping.\n")
        next
      }
      ...
      myBiomodProj <- BIOMOD_EnsembleForecasting(
        bm.em = current_model,
        bm.proj = NULL,
        proj.name = paste0("iter", iteration, "_", train_year, "_to_", test_year, "_", market_name),
        new.env = test_expl,
...
for (pair in year_pairs) {
  cat("  Train:", pair$train_year, "-> Test:", pair$test_year, "\n")
}

# Define base model variables (without odds - these will be added per market)
# NOTE: Odds columns are NOT included here - they will be added by get_market_model_vars
base_model_vars <- c(
...
for (market_name in names(market_results)) {
  ...
  calibration_plot <- ggplot(cal_data_combined, 
    ...
    labs(
      ...
      title = paste(market_name, "Market - Yearly Training (Mean Predictions)")
    ) +
    theme_minimal() +
...
if (length(market_results) > 1) {
  ...
  comparison_plot <- ggplot(combined_metrics, 
    ...
    labs(
      title = "Calibration Performance Across Markets",
      x = "Market",
