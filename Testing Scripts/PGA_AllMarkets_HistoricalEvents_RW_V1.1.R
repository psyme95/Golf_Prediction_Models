...
RETRAIN_FREQUENCY <- 42      # Retrain model every k events
NUM_ITERATIONS <- 30       # Number of times to repeat the process
...
# Function to get rolling training data
    ...
    for (test_event in first_test_event:total_events) {
      ...
      if (is.null(current_model) || events_since_training >= RETRAIN_FREQUENCY) {
        ...
        training_setup <- get_rolling_training_data(df_all, TRAINING_WINDOW_SIZE, available_vars, 
                                                    test_event, market_config$target_col, iteration)
  ...
  if (length(all_iterations_results) > 0) {
    ...
    averaged_predictions <- all_predictions_combined %>%
      ...
      summarise(
        ...
        Model_Score_Q1 = quantile(Model_Score, 0.25, na.rm = TRUE),
        Model_Score_Q3 = quantile(Model_Score, 0.75, na.rm = TRUE),
        Model_Score_SD = sd(Model_Score, na.rm = TRUE),
        Model_Score_IQR = IQR(Model_Score, na.rm = TRUE),
        ...
        GLM_Odds_Probability_Min = min(GLM_Odds_Probability, na.rm = TRUE),
        GLM_Odds_Probability_Max = max(GLM_Odds_Probability, na.rm = TRUE),
        GLM_Odds_Probability_Q1 = quantile(GLM_Odds_Probability, 0.25, na.rm = TRUE),
        GLM_Odds_Probability_Q3 = quantile(GLM_Odds_Probability, 0.75, na.rm = TRUE),
        ...
        GLM_ImpliedProb_Probability_Min = min(GLM_ImpliedProb_Probability, na.rm = TRUE),
        GLM_ImpliedProb_Probability_Max = max(GLM_ImpliedProb_Probability, na.rm = TRUE),
        GLM_ImpliedProb_Probability_Q1 = quantile(GLM_ImpliedProb_Probability, 0.25, na.rm = TRUE),
        GLM_ImpliedProb_Probability_Q3 = quantile(GLM_ImpliedProb_Probability, 0.75, na.rm = TRUE),
        ...
        Platt_Probability_Min = min(Platt_Probability, na.rm = TRUE),
        Platt_Probability_Max = max(Platt_Probability, na.rm = TRUE),
        Platt_Probability_Q1 = quantile(Platt_Probability, 0.25, na.rm = TRUE),
        Platt_Probability_Q3 = quantile(Platt_Probability, 0.75, na.rm = TRUE),
        Platt_Probability_SD = sd(Platt_Probability, na.rm = TRUE),
        
        # Static values (same across iterations)
...
df_all <- read_excel("./Weekly_Modelling/Input/Euro_Processed.xlsx")
...
for (market_name in names(market_results)) {
  ...
  cal_data_glm_odds <- create_calibration_data(result$averaged_predictions, "GLM_Odds_Probability_Mean", "GLM (Odds)")
  cal_data_glm_prob <- create_calibration_data(result$averaged_predictions, "GLM_ImpliedProb_Probability_Mean", "GLM (Implied Prob)")
  cal_data_platt <- create_calibration_data(result$averaged_predictions, "Platt_Probability_Mean", "Platt Scaling")
  ...
  calibration_plot <- ggplot(cal_data_combined, aes(x = predicted_prob, y = actual_rate, color = method)) +
    ...
    labs(x = "Predicted Probability",
         ...
         title = paste(market_name, "Market - Mean Predictions")) +
    theme_minimal() +
