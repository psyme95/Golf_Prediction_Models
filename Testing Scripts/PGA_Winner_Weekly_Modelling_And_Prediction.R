...
CV_REPETITIONS <- 1
CV_PERCENTAGE <- 0.7
MODEL_NAME <- paste0("Winner_Prediction_", format(Sys.time(), "%d%m"))

# Training parameters
TRAINING_WINDOW_SIZE <- 80  # Number of most recent events to use for training

# File paths - UPDATE THESE TO YOUR ACTUAL FILE PATHS
HISTORICAL_DATA_FILE <- "./Input/PGA.xlsx"
UPCOMING_EVENT_FILE <- "./Input/This_Week.csv"
...
df_upcoming <- read.csv(UPCOMING_EVENT_FILE)
df_upcoming <- df_upcoming[complete.cases(df_upcoming),]

# Get upcoming event details
upcoming_event_id <- "Upcoming_Event" # Assuming single event
...
# Add profit calculation to training data
train_data <- train_data %>%
  mutate(Win_Profit = (Win_odds - 1) * 10)

cat("Training data size:", nrow(train_data), "player-event combinations\n")

# Train the ensemble model
cat("Training ensemble model...\n")
trained_model <- train_ensemble_model(train_data, available_vars)

# Train calibration model
cat("Training calibration model...\n")
...
test_columns <- c("eventID", "surname", "firstname", "Win_odds", "rating", available_vars)
...
cat("Making predictions for", nrow(prediction_data), "players in upcoming event...\n")
...
myBiomodProj <- BIOMOD_EnsembleForecasting(
  bm.em = trained_model,
  bm.proj = NULL,
  proj.name = paste0("upcoming_", upcoming_event_id),
  new.env = test_expl,
...
essential_cols <- c("surname", "firstname", "Win_odds", "rating")
...
# Apply calibration
cat("Applying calibration to predictions...\n")
calibration_data <- data.frame(
  Model_Score = predictions$Model_Score,
  Win_odds = predictions$Win_odds
)
