...
CV_REPETITIONS <- 1
CV_PERCENTAGE <- 0.7
MODEL_NAME <- paste0("Win_RW_", format(Sys.time(), "%d%m_%H%M"))
...
get_rolling_window_data <- function(df_all, current_event, window_size, available_vars) {
  ...
  train_data <- df_all %>%
    ...
    select(playerID, eventID, posn, win, Win_odds, all_of(available_vars))
...
simulate_kelly_betting_custom <- function(df, 
  ...
  for(event_idx in 1:length(unique_events)) {
    ...
    for(i in 1:nrow(event_data)) {
      ...
      if (is.na(row$Calibrated_Probability) || is.na(row$Win_odds) || 
          row$Win_odds <= 0 || !row$Valid_odds) {
        next
      }
      
      # Calculate probability edge over market
      implied_prob <- 1 / row$Win_odds
      ...
      passes_filters <- (
        adjusted_kelly > min_kelly_threshold &&
          ...
          row$Win_odds >= min_odds &&
          row$Win_odds <= max_odds
      )
    ...
    if(nrow(potential_bets) > 0) {
      ...
      for(j in 1:nrow(potential_bets)) {
        ...
        won_bet <- original_row$Actual_Win == 1
        
        if(won_bet) {
          profit <- (bet_data$final_bet_amount / 10) * original_row$Win_Profit
        } else {
...
for (current_event in MIN_PREDICTION_EVENT:total_events) {
  ...
  if (need_retrain || length(trained_models) == 0) {
    ...
    logit_model_data <- data.frame(
      ...
      Actual_Win = train_data$win,
      Win_odds = train_data$Win_odds
    )
        
     # Train calibration model
     calibration_model <- glm(Actual_Win ~ Model_Score + Win_odds,
                              data = logit_model_data,
                              family = binomial())
      
     # calibration_model <- glm(Actual_Win ~ Model_Score,
     #                          data = logit_model_data,
     #                          family = binomial())
  }
  ...
  test_columns <- c("playerID", "eventID", "posn", "win", "Quality", 
                    "Win_odds", "Win_Profit", "rating", available_vars)
  ...
  essential_cols <- c("eventID", "playerID", "posn", "win")
  merge_cols <- intersect(names(test_data), c(essential_cols, "Win_odds", "Win_Profit", "rating", "Quality"))
  ...
  calibration_data <- data.frame(
    ...
    Win_odds = test_prediction$Win_odds
  )
  ...
  test_prediction$Valid_odds <- test_prediction$Win_odds > 1
  test_prediction$Model_Rank <- rank(-test_prediction$Model_Score, ties.method = "first")
  test_prediction$Prediction_Correct <- ifelse(
    (test_prediction$win == 1 & test_prediction$Model_Rank <= 1) | 
      (test_prediction$win == 0 & test_prediction$Model_Rank > 1), 
    1, 0)
