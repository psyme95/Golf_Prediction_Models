...
# Function to get training data from previous N calendar years
get_training_data <- function(df_historical, training_years, available_vars) {
  # Get the most recent event date to determine the "current" year
  max_date <- max(df_historical$Date, na.rm = TRUE)
  current_year <- year(max_date)
  
  # Define the training period: previous N complete calendar years
  # e.g., if current_year is 2025 and training_years is 2, use 2023-2024
  end_year <- current_year - 1
  start_year <- current_year - training_years
  
  training_start <- as.Date(paste0(start_year, "-01-01"))
  training_end <- as.Date(paste0(end_year, "-12-31"))
  
  cat("Current year:", current_year, "\n")
  cat("Training period:", start_year, "to", end_year, "\n")
  cat("Date range:", as.character(training_start), "to", as.character(training_end), "\n")
  
  # Filter events within the training period
  train_events <- df_historical %>%
    filter(Date >= training_start & Date <= training_end)
  
  # Get unique events for reporting
  unique_events <- train_events %>%
    select(eventID, Date) %>%
    distinct() %>%
    arrange(Date)
  
  total_events_all <- df_historical %>%
    select(eventID) %>%
    distinct() %>%
    nrow()
  
  cat("Training on", nrow(unique_events), "events from", start_year, "-", end_year, 
      "out of", total_events_all, "total events\n")
...
process_tournament_training <- function(tour_key, tour_info, base_model_vars) {
  ...
  training_result <- get_training_data(df_historical, TRAINING_YEARS, base_model_vars)
  ...
  if (length(market_training_results) > 0) {
    ...
    summary_file <- paste0(results_dir, tour_key, "_Training_Summary_", MODEL_SUFFIX, ".xlsx")
    ...
    models_file <- paste0(results_dir, tour_key, "_Trained_Models_", MODEL_SUFFIX, ".rds")
...
if (length(tournament_results) > 0) {
  ...
  cat("Training window:", TRAINING_YEARS, "previous calendar years\n")
